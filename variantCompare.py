...
def create_options_parser():
	...
	inputRV = type=RegexValidator("^((\w+):)?[a-zA-Z0-9,\.'()_\s-]+\.*[a-zA-Z0-9,\s-]*$",
	...
	parser.add_argument('-s','--set-operation',
			help="""Specify a set operation between one or more input files. Possible operations are:
			union ([uU]), intersect ([iI]), and complement ([cC]). Set operations are formatted as
			follows: <name>=<operation>(<input_name>:<sample_id>,<input_name>:<sample_id>,etc.). Here
			is an example intersect on two unions:
			\'-s out1=u(fId1:sId1,fId1:sId2,fId2:sId3) -s out2=u(fId3:sId4,fId3:sId5)
			-s out3=i(out1,out2)\'. An \'fId\' refers to a named file (see --input) and an \'sId\'
			refers to a sample within that file.""")
